	...
	int mph;			// speed of car
{
	...
	while(1) {
		...
		Wait(shm.carPosSem[(((from) == WEST) ? 0 : NUMPOS-1)]);
		Wait(shm.mutex);

		if (shm.carNum[1 - from] > 0 || shm.carNum[from] > 0 && shm.carLineNum[1 - from] > 0 ) {
	
			Signal(shm.carPosSem[(((from) == WEST) ? 0 : NUMPOS-1)]);
			shm.carLineNum[from]++;
	
			Signal(shm.mutex);

			Wait(shm.semDirect[from]);
	...
	for (i = 1; i < NUMPOS; i++) {
		...
		Delay (3600/mph);
		Wait(shm.carPosSem[np - 1]);
		ProceedRoad ();
		Signal(shm.carPosSem[p - 1]);
		PrintRoad ();
	...
	shm.carNum[from]--;
	if (shm.carNum[from] == 0 && shm.carLineNum[1 - from] > 0 && shm.carLineNum[from] == 0) {

		for (int i = 0; i < shm.carLineNum[1-from]; i++) {
	
			Signal(shm.semDirect[1 - from]);
		}
	}
	else {
		Signal(shm.semDirect[1 - from]);
